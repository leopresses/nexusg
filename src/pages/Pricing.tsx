...
import { Check, Zap, Crown, Rocket, Building } from "lucide-react";
...
const plans = [{
  ...
  features: ["Até 3 clientes", "Tarefas ilimitadas", "Relatórios avançados", "White-label (sua marca)", "Suporte prioritário"],
  highlighted: true
}, {
  id: "elite",
  name: "Elite",
  ...
  features: ["Até 10 clientes", "Tudo do Pro", "API de integração", "Múltiplos usuários", "Dashboard personalizado", "Suporte 24/7"],
  highlighted: false
}, {
  id: "agency",
  name: "Agency",
  ...
  features: ["Clientes ilimitados", "Tudo do Elite", "Gerente de conta dedicado", "Treinamento personalizado", "SLA garantido", "Integrações customizadas"],
  highlighted: false
}];
export default function Pricing() {
  ...
  const handleUpgrade = (planId: string, planName: string, planPrice: string, clientsLimit: string) => {
    const message = encodeURIComponent(`Olá! Quero contratar/upgrade no Gestão Nexus.\n\n` + `Plano de interesse: ${planName}\n` + `Preço: ${planPrice}/mês\n` + `Limite: ${clientsLimit}\n` + `Meu e-mail: ${userEmail}\n\n` + `Pode me ajudar com o processo?`);
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${message}`, '_blank');
  };
  return <AppLayout title="Planos e Assinaturas" subtitle="Escolha o plano ideal para o seu negócio">
      <div className="space-y-8">
        {/* Current Plan Banner */}
        <motion.div className="rounded-xl bg-gradient-to-r from-primary/10 to-accent/10 border border-primary/20 p-6" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-muted-foreground">
                ...
                {profile?.clients_limit && <span className="ml-2">
        ...
        delay: 0.1
          ...
          return <motion.div key={plan.id} className={`rounded-xl border p-6 flex flex-col ${plan.highlighted ? 'bg-card border-primary shadow-lg shadow-primary/10 relative' : 'bg-card border-border'} ${isCurrentPlan ? 'ring-2 ring-primary' : ''}`} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            delay: 0.1 + index * 0.1
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className={`h-10 w-10 rounded-lg flex items-center justify-center ${plan.highlighted ? 'bg-primary/20' : 'bg-muted'}`}>
                ...
                {isCurrentPlan ? <Button variant="outline" className="w-full" disabled>
                    Plano Atual
                  </Button> : plan.id === "starter" ? <Button variant="outline" className="w-full" disabled>
                    Plano Gratuito
                  </Button> : <Button variant={plan.highlighted ? "default" : "outline"} className="w-full gap-2" onClick={() => handleUpgrade(plan.id, plan.name, plan.price, plan.clientsLimit)}>
        ...
        <motion.div className="rounded-xl bg-card border border-border p-6 text-center" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
