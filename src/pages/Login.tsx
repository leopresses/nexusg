...
export default function Login() {
  ...
  // Evita recriar objeto em renders (micro otimização + padronização)
  const toastStyle = useMemo(() => TOAST_STYLE, []);
  ...
  }, [user, navigate, toast, toastStyle]);
  ...
  const handleOAuthSignIn = useCallback(
    async (provider: OAuthProvider) => {
      ...
      if (error) {
        ...
        toast({
          title: `Erro ao entrar com ${providerName}`,
  ...
  const handleSubmit = useCallback(
    ...
    [email, password, isLoading, signIn, toast, toastStyle, navigate],
  );
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="flex h-12 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-12 py-2 text-sm text-slate-900 placeholder:text-slate-400 shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2"
                  required
                />

                {/* Botão único (o olhinho nativo do navegador é removido via CSS global) */}
                <button
                  type="button"
