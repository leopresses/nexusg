...
import { Loader2, CheckCircle2, XCircle, AlertCircle, RefreshCw, LogOut, ExternalLink, Clock } from "lucide-react";
  ...
  const getErrorMessage = (code: string, details?: string): string => {
    const messages: Record<string, string> = {
      ...
      redirect_uri_mismatch:
        "Erro de configuração OAuth. O URI de redirecionamento não está autorizado no Google Cloud Console.",
      ...
      <Card>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Conecte sua conta para sincronizar métricas automaticamente</CardDescription>
              </div>
            </div>
            {getStatusBadge()}
          </div>
        ...
        <CardContent className="space-y-4">
          {connection?.status === "connected" && (
            <>
              ...
              <div className="flex gap-2">
                <Button variant="outline" onClick={syncMetrics} disabled={isSyncing} className="flex-1">
                  {isSyncing ? (
            ...
            <>
              <div className="rounded-lg bg-muted/50 p-4">
                <p className="text-sm text-muted-foreground">
                  Conecte com Google Place ID para sincronizar todas as informações referentes ao negócio de seu
                  cliente.
                </p>
              </div>
          ...
          <p className="text-xs text-muted-foreground">
            A sincronização automática ocorre diariamente às 06:00. Você também pode sincronizar manualmente a qualquer
            momento.
          </p>
        </CardContent>
      </Card>
